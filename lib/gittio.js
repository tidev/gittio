        ...
        dist.download(dst.dist, function(err, tmpPath) {
          ...
          if (cmp.type === 'widget') {
            ...
            var pkgs = glob.sync('*/*/*/package.json', {
              ...
              return fs.readJSONSync(path.join(dst.trgPath, pkgPath));
            });
...
function demo(o) {
  ...
  component.lookup(o.id, function(err, cmp) {
    ...
    // create project
    exec('ti', ['create', '-t', 'app', '-u', 'http://gitt.io', '-p', cmp.platforms.join(','), '-n', project_name, '--id', cmp.id, '-d', process.cwd()], null, function() {
