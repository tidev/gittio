...
function _uninstallModule(o) {
  ...
  config.current_modules.map(function(m) {
    ...
    if (m.name === o.id && (o.version === undefined || o.version === m.version) && (o.platform === undefined || o.platform === m.platform)) {
      tiapp.dropDependency(o.id, o.version, o.platform);
  ...
  component.lookup(o.id, function(err, info) {
    ...
    if (installed && !o.force) {
      ...
      if (info.type === "module") {
        ...
        alloy.addDependency(o.id, target.version, target.platforms);
      }
      return;
    }

    if (!target || target.dist === null) {
        logger.error(o.id + (o.version ? '@' + o.version : "") + " distributable not available in repository.");
        return;
    }
