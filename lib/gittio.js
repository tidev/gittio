    ...
    _.each(dists, function(dst) {
      ...
      if (installed && !o.force) {
        ...
        dist.download(dst.dist, function(err, tmpPath) {
          ...
          // copy from tmpPath
          fs.copyDir(path.join(tmpPath, dst.srcPath), dst.trgPath, function(err) {
...
function demo(o) {
  ...
  component.lookup(o.id, function(err, cmp) {
    ...
    // create project
    exec('ti', ['create', '-t', 'app', '-u', 'http://gitt.io', '-p', cmp.platforms.join(','), '-n', cmp.id, '--id', cmp.id, '-d', process.cwd()], null, function() {
