...
function _installSingle(id, version) {
	...
	request(url, function(error, response, body) {
		...
		if (!error && response.statusCode == 200) {
			...
			var url = "https://nodeload.github.com/" + info.user + "/" + info.repo + "/zip/" + info.branch;

			download(url, function(err, tmpPath) {
				...
				var trgPath = path.join(process.cwd(), 'app', 'widgets', id);
				...
				fs.mkdirs(trgPath, function(err) {
					...
					var srcPath = path.join(tmpPath, info.repo + '-' + info.branch, plfPath);
