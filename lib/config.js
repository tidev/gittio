    ...
    if (err) {
      ...
      if (err === 'Not Found') {
        ...
        if (err.indexOf('[Error: Module version mismatch.]') !== -1) {
          logger.error('You need to rebuild the AppC/Titanium CLI after switching major NodeJS version: [sudo] npm i -g titanium && appc use latest --force');
        } else if (err.indexOf('(node) sys is deprecated. Use util instead.')) {
