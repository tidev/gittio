...
exports.initDist = function(cmp, dist) {

  // init paths
  if (cmp.type == 'widget') {
    dist.trgPath = path.join(config.widgets_path, cmp.id);

    if (dist.path.length > 0) {
      dist.srcPath = path.join(cmp.repo + "-" + dist.tree, dist.path);
    } else {
      dist.srcPath = path.join(cmp.repo + "-" + dist.tree);
    }

  } else {
    dist.trgPath = o.global ? config.global_modules_path : config.modules_path;
    dist.srcPath = 'modules';
  }
};